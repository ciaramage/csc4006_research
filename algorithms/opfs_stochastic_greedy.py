...
def STOCHASTIC_OPFS(X, Nc=1, with_replacement=True, percentage=0):
    ...
    for j in range(0, Nc):
        ...
        # variance explained using orthogonalization
        '''Qv = gram_schmidt(np.atleast_2d(curr_Y[:, idx]).T)
        Rv = np.zeros(curr_Y.shape)
        for i in range(0, curr_Y.shape[1]):
            Rv[:,i] = np.matmul(np.matmul(Qv, Qv.T), curr_Y[:,i])'''
        
        # variance explained using deflation
